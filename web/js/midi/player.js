...
var scheduleTracking = function(channel, note, technical, currentTime, offset, message, velocity, time) {
	return setTimeout(function() {
		...
		///
		///
		if (midi.currentTime === queuedTime && queuedTime < midi.endTime) { // grab next sequence
...
var startAudio = function(currentTime, fromCache, onsuccess) {
	...
	for (var n = 0; n < length && messages < 100; n++) {
		var obj = data[n];
		queuedTime += obj[1]
		if ((queuedTime < currentTime) || (queuedTime == currentTime && obj[0].event.subtype !== 'noteOn')) {
			offset = queuedTime;
			continue;
		} else if (queuedTime == currentTime) {
			// console.debug("//Fix first note not played at " + currentTime + " Queud Time is : " + queuedTime)
			// console.debug(obj)
		}
		///
		...
		switch (event.subtype) {
			...
			case 'noteOn':
                ...
                technical = event.technical; // Hack to add string, fret
				eventQueue.push({
				    event: event,
				    time: queueTime,
				    source: MIDI.noteOn(channelId, event.noteNumber, event.velocity, delay),
				    interval: scheduleTracking(channelId, note, technical, queuedTime + midi.startDelay, offset - foffset, 144, event.velocity)
				});
				messages++;
				break;
			case 'noteOff':
				...
				eventQueue.push({
				    ...
				    interval: scheduleTracking(channelId, note, technical, queuedTime, offset - foffset, 128, 0)
				});
				break;
			default:
				break;
